...
const Tela8 = () => {
  ...
  const [novoMaterial, setNovoMaterial] = useState<NovoMaterialForm>({
    materia: "",
    unidade: "",
    preco: "",
  });
  ...
  const handleSalvarMaterial = async () => {
    ...
    if (isNaN(precoNum) || precoNum < 0) {
      setFormError(
        'Preço inválido. Use números e opcionalmente "." ou "," como decimal.'
      );
      return;
    }
  ...
  const handleExcluirMaterial = async (id: number | undefined) => {
    ...
    if (
      !window.confirm(
        `Tem certeza que deseja excluir o material ID ${id}? Esta ação não pode ser desfeita.`
      )
    ) {
    ...
    try {
      ...
      const error = err as AxiosError<{ error?: string; details?: string }>;
      console.error(
        `Erro ao excluir material ID ${id}:`,
        error.response?.data || error.message
      );
      if (error.response?.status === 404) {
        ...
        // Conflito (FK violation / em uso)
        setDeleteError(
          error.response.data?.error ||
            `Não foi possível excluir o material ID ${id} (pode estar em uso em algum chamado).`
        );
      } else {
        setDeleteError(
          `Erro ao excluir material ID ${id}. Tente novamente mais tarde.`
        );
      }
    }
  };
      ...
      <div className="lateral_tela_8">
        <div className="descricao_tela_8">
          ...
          <div className="tiulo_descricao_tela_8">
            <div className="inicio_tela_8">Material</div>
          ...
          <div className="itens_descricao_tela_8">
            ...
            {!loading &&
              materiais.map((material) => (
                <div className="item_tela_8" key={material.id}>
                  ...
                  <div className="unidade">{material.unidade}</div>
                  <div className="preco">
            ...
            {adicionando && (
              // Dentro do {adicionando && (...)}
              <div className="item_tela_8_item-adicionar">
                {/* Adicione a classe base .item_tela_8 aqui também */}
                <div className="materia">
                  <input
                    type="text"
                    ...
                    style={{ width: '80%', padding: '5px', backgroundColor: 'white', borderRadius: '1vw' }}
                    className="form-input" // Classe para o input
                    autoFocus
                  />
                </div>
                ...
                <div className="actions-form">
                  {" "}
                  {/* Adiciona uma classe para a área de ações */}
                  {formError && (
                    <div className="form-error-inline">{formError}</div>
                  )}
                  <button
